...
def get_action(p_type, arch, robot_idx, state, model):
    if p_type == "dummy": return [0.0, 0.0]
...
def main():
    ...
    m1 = load_ai_model(MODEL_1_PATH, PLAYER_1_ARCH, DEVICE) if PLAYER_1_TYPE == "ai" else None
    m2 = load_ai_model(MODEL_2_PATH, PLAYER_2_ARCH, DEVICE) if PLAYER_2_TYPE == "ai" else None
    ...
    while True:
        ...
        while not done and step_count < MAX_STEPS:
            ...
            r2_s = get_reward(None, info, done, state[1], info.get('is_collision', False))
            total_r1 += r1_s; total_r2 += r2_s
            ...
            if step_count % 10 == 0:
                ...
                ax.relim()
                ax.autoscale_view()
                ax.set_title(f"Round {round_count+1} | {PLAYER_1_ARCH} vs {PLAYER_2_ARCH}")
                fig.canvas.flush_events()

            sys.stdout.write(f"\rStep: {step_count:4d} | R1 ({PLAYER_1_ARCH}): {total_r1:7.1f} | R2 ({PLAYER_2_ARCH}): {total_r2:7.1f}")
            sys.stdout.flush()
        ...
        if PLAYER_1_TYPE == "ai": m1 = load_ai_model(MODEL_1_PATH, PLAYER_1_ARCH, DEVICE)
        if PLAYER_2_TYPE == "ai": m2 = load_ai_model(MODEL_2_PATH, PLAYER_2_ARCH, DEVICE)
