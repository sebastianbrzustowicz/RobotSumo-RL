...
import os, random, pygame, glob
from collections import deque
from src.env.sumo_env import SumoEnv
from src.agents.PPO.agent import create_agent, update_policy, get_distribution, calculate_returns
...
def train():
    ...
    c_list = [
        (0.51, 40, 50), (0.52, 36, 49), (0.53, 32, 48), (0.54, 28, 47),
        (0.55, 24, 46), (0.56, 20, 45), (0.57, 16, 44), (0.58, 12, 43),
        (0.59, 8, 42), (0.60, 5, 40)
    ]
    ...
    try:
        for ep in range(cfg['episodes']):
            ...
            while not done:
                ...
                s_t = torch.as_tensor(obs[0], dtype=torch.float32, device=device).unsqueeze(0)
                opp_s_t = torch.as_tensor(obs[1], dtype=torch.float32, device=device).unsqueeze(0)
            ...
            with torch.no_grad():
                _, last_v = model(torch.as_tensor(obs[0], dtype=torch.float32, device=device).unsqueeze(0))
            
            rets = calculate_returns(ep_data["r"], cfg['gamma'], last_v.item(), ep_data["m"]).to(device)
            ...
            print(f"Ep {ep:04d} | Steps: {ep_steps:4} | vs {opp_name:12} | Reward: {ep_rew:7.2f} | WR: {wr:.2%} | {status}")

            # --- NETWORK UPDATE (PPO) ---
            if buffer_steps >= cfg['update_every_steps']:
                adv_b = torch.cat(storage["ad"]).detach()
                update_policy(model, optimizer, torch.cat(storage["s"]), torch.cat(storage["a"]), 
                              torch.cat(storage["lp"]).detach(), (adv_b - adv_b.mean())/(adv_b.std() + 1e-8), 
                              torch.cat(storage["rt"]).detach(), cfg['ppo_epochs'], cfg['eps_clip'], cfg['entropy_coef'])
