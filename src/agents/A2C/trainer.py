...
def train():
    ...
    last_update_ep = 0
    print(f"🚀 Start A2C Training | LR: {cfg['lr']} | Gamma: {cfg['gamma']} | Device: {DEVICE}")

    for ep in range(cfg["episodes"]):
        history_files = get_history_models()
        is_fighting_master = random.random() >= 0.20 or not history_files
        path = cfg["master_path"] if is_fighting_master else random.choice(history_files)
        ...
        for step in range(cfg["max_steps"]):
            ...
            if done:
                ...
                print(f"Ep {ep+1:04d} | Steps: {step+1:4} | vs {opp_name:12} | Reward: {ep_data['reward_sum']:7.2f} | WR: {wr:.2%} | {status}")
                break
